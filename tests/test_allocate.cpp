...
protected:
    ...
    static void* get_header_from_user_ptr(void* user_ptr) {
        ...
        auto* seg = MappedSegment::get_segment(user_ptr);
        if (seg->page_descriptors_[0].status == PageStatus::HUGE_SLAB) {
            return seg; // For huge objects, the "header" is the segment itself
        }
        return static_cast<char*>(user_ptr) - sizeof(LargeSlabHeader);
    }
};
...
TEST_F(AllocateTest, AllocateAndVerifyMultiPageLargeObject) {
    const size_t user_size = MAX_SMALL_OBJECT_SIZE + 100;
    ...
    MappedSegment* seg = MappedSegment::get_segment(header_ptr);
    for (uint16_t i = 0; i < expected_pages; ++i) {
        SCOPED_TRACE("Verifying page " + std::to_string(i));
        void* current_page_ptr = static_cast<char*>(header_ptr) + i * PAGE_SIZE;
        PageDescriptor* desc = seg->get_page_desc(current_page_ptr);
        
        EXPECT_EQ(desc->status, PageStatus::LARGE_SLAB);
...
TEST_F(AllocateTest, AllocationSpansAcrossSegments) {
    ...
    const size_t available_pages = (SEGMENT_SIZE / PAGE_SIZE) - metadata_pages;
    
    // 请求一个几乎占满整个 Segment 可用空间的用户区
    const size_t user_size1 = available_pages * PAGE_SIZE - header_size;
    
    void* user_ptr1 = heap_->allocate(user_size1);
    ASSERT_NE(user_ptr1, nullptr);
    MappedSegment* seg1 = MappedSegment::get_segment(user_ptr1);

    // 再请求一个很小的 Large Object，freelist 中没有合适的，应创建新 Segment
    void* user_ptr2 = heap_->allocate(MAX_SMALL_OBJECT_SIZE + 1);
    ASSERT_NE(user_ptr2, nullptr);
    MappedSegment* seg2 = MappedSegment::get_segment(user_ptr2);
...
TEST_F(AllocateTest, HandlesInvalidOrEdgeCaseSizes) {
    ...
    const size_t huge_size = SEGMENT_SIZE * 2;
    void* user_ptr = heap_->allocate(huge_size);
    ASSERT_NE(user_ptr, nullptr);

    // For huge objects, there's no embedded header. The "header" is the segment itself.
    MappedSegment* seg = MappedSegment::get_segment(user_ptr);
    PageDescriptor* desc = &seg->page_descriptors_[0];
    EXPECT_EQ(desc->status, PageStatus::HUGE_SLAB);
...
TEST_F(AllocateTest, AllocateObjectAtHugeThreshold) {
    const size_t header_size_seg = sizeof(MappedSegment);
    const size_t metadata_pages = (header_size_seg + PAGE_SIZE - 1) / PAGE_SIZE;

    const size_t header_size_large = sizeof(LargeSlabHeader);
    const size_t large_metadata_pages = (header_size_large + PAGE_SIZE - 1) / PAGE_SIZE;  

    const size_t available_pages = (SEGMENT_SIZE / PAGE_SIZE) - metadata_pages - large_metadata_pages;
    const size_t huge_object_threshold = available_pages * PAGE_SIZE;
    
    void* user_ptr = heap_->allocate(huge_object_threshold);
    ASSERT_NE(user_ptr, nullptr);

    MappedSegment* seg = MappedSegment::get_segment(user_ptr);
    PageDescriptor* desc = seg->get_page_desc(user_ptr);
...
TEST_F(AllocateTest, InterleaveLargeAndHugeAllocations) {
    void* large_ptr1 = heap_->allocate(MAX_SMALL_OBJECT_SIZE + 1);
    ASSERT_NE(large_ptr1, nullptr);
    MappedSegment* regular_seg1 = MappedSegment::get_segment(large_ptr1);

    void* huge_ptr = heap_->allocate(SEGMENT_SIZE);
    ASSERT_NE(huge_ptr, nullptr);
    MappedSegment* huge_seg = MappedSegment::get_segment(huge_ptr);
    
    void* large_ptr2 = heap_->allocate(MAX_SMALL_OBJECT_SIZE + 2);
    ASSERT_NE(large_ptr2, nullptr);
    MappedSegment* regular_seg2 = MappedSegment::get_segment(large_ptr2);

    EXPECT_NE(huge_seg, regular_seg1);
    EXPECT_NE(huge_seg, regular_seg2);

    EXPECT_EQ(huge_seg->page_descriptors_[0].status, PageStatus::HUGE_SLAB);
    EXPECT_EQ(regular_seg1->get_page_desc(large_ptr1)->status, PageStatus::LARGE_SLAB);
    EXPECT_EQ(regular_seg2->get_page_desc(large_ptr2)->status, PageStatus::LARGE_SLAB);

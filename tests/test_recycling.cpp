...
TEST_F(RecyclingTest, ReuseFreedLargeSlabOfSameSize) {
    // 1. 准备: 请求一个明确的大对象尺寸
    const size_t size = MAX_SMALL_OBJECT_SIZE + 1;
...
TEST_F(RecyclingTest, PageDescriptorsAreMarkedAsFreeAfterRelease) {
    // 1. 准备: 请求一个明确的大对象尺寸
    const size_t size = MAX_SMALL_OBJECT_SIZE + 1024;
    ...
    for (size_t i = 0; i < num_pages; ++i) {
        char* page_ptr = static_cast<char*>(ptr) + i * PAGE_SIZE;
        PageDescriptor* desc = seg->get_page_desc(page_ptr);
...
TEST_F(RecyclingTest, ReuseMemoryFromAnEmptiedSmallSlabForLargeAllocation) {
    ...
    void* slab_address = MappedSegment::get_segment(small_ptrs[0])->get_page_desc(small_ptrs[0])->slab_ptr;
    ...
    const size_t large_alloc_size = num_pages_for_slab * PAGE_SIZE;
    if (large_alloc_size <= MAX_SMALL_OBJECT_SIZE) {
